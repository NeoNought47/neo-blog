...
## I 搭建策略
### 1. 常规化**原理**：源码和数据都在同一个服务器里
**影响**：N/A 常规安全测试手法

### 2. 账库分离**原理**：源码和数据库在不同的服务器
**存储**：其他服务武器上面的数据库&云数据库产品
**影响**：数据被单独存放，能连接才能影响数据

### 3. 前后端分离**原理**：前端JS框架，API传输数据
前端站点  
...
## II Docker虚拟化技术： Docker使用Linux内核的命名空间和控制组（cgroup）等技术来创建隔离的容器。==这种隔离性允许容器在相同的宿主系统上运行，同时保持与其他容器和宿主系统的独立性==。
...
影响：攻击者虚拟空间磁盘
安全影响：
1. 攻击者虚拟空间磁盘： 如果攻击者能够***获得对容器的控制权***，他们可能尝试在容器中执行各种攻击，包括对容器内的虚拟磁盘空间的篡改、==删除或读取敏感信息==。
...
即使使用了自己的域名，网站程序和服务器仍可能属于建站平台。==所以安全测试中，不能只看网页上写着谁的公司名称，而要确认：域名、服务器、应用程序、账号和数据分别归谁所有，以及授权范围具体覆盖哪一部分。== 所以攻击的可能是网站提供商

## IV 静态&伪静态

### 静态web例子：大学学的html设计的网站
原理：数据没有传输性（js传输不算）
影响：无漏洞
...
无漏洞： 如果静态网站没有包含漏洞或恶意注入，它们通常是相对安全的。然而，开发者仍然需要注意HTML、CSS和JavaScript中的潜在漏洞，例如==跨站脚本（XSS）攻击==。
